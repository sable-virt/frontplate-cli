...
const Util = {
    ...
    getConfig(program) {
        let config = require('../frp.config')(program.production || false);
        let localConfig = path.join(process.cwd(),'frp.config.js');
        if (this.exists(localConfig)) {
            config = merge(config,require(localConfig)(program.production || false));
        }
        if(program.config) {
            config = merge(config,require(program.config)(program.production || false));
        }
        return config
    }
};
